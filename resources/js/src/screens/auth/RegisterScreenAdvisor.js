...
import {AuthContext, onAuth, advisorsignUp} from './auth';
...
const RegisterScreen = () => {                              
  const history = useHistory();
    ...
    const [cmpnyFK, setcmpnyFK] = React.useState(2);
    const [roleFK, setroleFK] = React.useState(3);
    ...
    const submitForm=()=>{
      ...
      const data={name,username,password,password2,cmpnyFK,roleFK};
      advisorsignUp(data).then(({data,message})=>{
        setTimeout(()=>{
          hideloader();
