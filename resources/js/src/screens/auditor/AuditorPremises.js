...
//   const {subcr,cmpny} = useContext(SubscriptionContext);
  ...
  const RenderItem=({data}) =>  Object.keys(data).map((pg) => {
 return <List.Item  key={pg}>
  ...
  {data&&<List  ordered divided><RenderItem data={data}/></List>}
...
export default AuditorPremises