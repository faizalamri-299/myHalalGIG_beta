...
import {UploadHASContext,
        postHASProductHalalCert,
        deleteHASProductHalalCert,
        downloadHASProductHalalCert} from './UploadHAS';

import pdfMake from "pdfmake/build/pdfmake";
import * as pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake ? pdfFonts.pdfMake.vfs : pdfMake.vfs;

const TabHASProductHalalCert = () => {
  
  const {HASProductHalalCert} = useContext(UploadHASContext);
  ...
  const uploadFile=()=>{
    ...
    formData.append("HASProductHalalCert",filename); 
    formData.append("date",date); 
    formData.append("refno",refno); 
    // Details of the uploaded file   
    postHASProductHalalCert(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHASProductHalalCert(pk).then(console.log(pk));
  }
      ...
      <div className="in innerContainer">
        ...
        <Form.Input
         ...
         <Form.Input
          ...
          {HASProductHalalCert &&
            <RenderHAS data={HASProductHalalCert}/>
          }
        <Divider>
    </Divider>
      </div>
...
export default TabHASProductHalalCert