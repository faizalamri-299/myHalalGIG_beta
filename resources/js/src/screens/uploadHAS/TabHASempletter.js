...
import {UploadHASContext,
        postHASEmpletter,
        deleteHASEmpletter} from './UploadHAS';

import pdfMake from "pdfmake/build/pdfmake";
import * as pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake ? pdfFonts.pdfMake.vfs : pdfMake.vfs;

const TabHASempletter = () => {
  
  const {HASempletter} = useContext(UploadHASContext);
  let { path, url } = useRouteMatch();
  const [empletter_filename, setfilename] = React.useState(null);
  const [date, setdate] = React.useState(null);
  const [refno, setrefno] = React.useState(null);

  const uploadFile=()=>{
    const formData = new FormData(); 
    formData.append("HASempletter",empletter_filename); 
    formData.append("date",date); 
    formData.append("refno",refno);
    // Details of the uploaded file   
    postHASEmpletter(formData).then(x=>{
      console.log(x)
  ...
  const RenderHAS = props => {
    ...
    <Table.Row >
     ...
     <Button.Group basic floated='right' size='small'>
      <Popup content='Muat Turun Fail'position='top center' trigger={<Button href={"/files/HASFILE/"+x.cmpnyFK+"/"+x.empletter_filename} target="_blank"  icon='download' />} />
      <Popup content='Padam Fail' position='top center'  trigger={<Button onClick={()=>deleteFile(x.id)} icon='trash alternate' />} />
        
      </Button.Group>
       {/* <Button size='mini' compact basic float='right'color="teal" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.empletter_filename} target="_blank">Muat Turun</Button>
     <Button size='mini' compact basic color="red" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.empletter_filename} target="_blank">Padam</Button> */}
     </Table.Cell>
   </Table.Row>
    );
    return <Table>
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Surat Pelantikan</Header> 
...
export default TabHASempletter