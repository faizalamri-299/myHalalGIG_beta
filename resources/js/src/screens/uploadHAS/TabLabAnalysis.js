...
const TabLabAnalysis = () => {
  ...
  const {HASLabAnalysis} = useContext(UploadHASContext);
  let { path, url } = useRouteMatch();
  const [hla_file_name, setChecklist] = React.useState(null);
  const [date, setdate] = React.useState(null);
  const [refno, setrefno] = React.useState(null);

  const uploadFileChecklist=()=>{
    const formData = new FormData(); 
    formData.append("HASLabAnalysis",hla_file_name); 
  ...
  const RenderHASRawMat = props => {
    ...
    <Table.Row >
     <Table.Cell>{x.hla_file_name}</Table.Cell>
     ...
     <Button.Group basic floated='right' size='small'>
      ...
      <Popup content='Padam Fail' position='top center'  trigger={<Button onClick={()=>deleteChecklist(x.id)} icon='trash alternate' />} />
       ...
       {/* <Button size='mini' compact basic float='right'color="teal" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.hla_file_name} target="_blank">Muat Turun</Button>
     <Button size='mini' compact basic color="red" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.hla_file_name} target="_blank">Padam</Button> */}
     </Table.Cell>
   </Table.Row>
    );
    return <Table>
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Analisis Makmal</Header> 
        <Form.Group style={{display:'flex'}}>
        <Form.Input
            label='Fail'
            type="file"
            onChange={e=>setChecklist(e.target.files[0])}
          /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
         <Form.Input
              label='No Rujukan'
              ...
              value={refno}
            />
          <Button icon labelPosition='right' floated='right' onClick={() => {uploadFileChecklist();}}>
          Muat Naik
            <Icon name='right arrow' />
          </Button>
        </Form.Group>
          {HASLabAnalysis &&
            <RenderHASRawMat data={HASLabAnalysis}/>
          }
        <Divider>
      {/* <Header as='h4'>
