...
const TabRawMaterial = () => {
  ...
  const {HASChecklist} = useContext(UploadHASContext);
  let { path, url } = useRouteMatch();
  const [hc_file_name, setChecklist] = React.useState(null);
  const [date, setdate] = React.useState(null);
  const [refno, setrefno] = React.useState(null);

  const uploadFileChecklist=()=>{
    const formData = new FormData(); 
    formData.append("HASChecklist",hc_file_name); 
  ...
  const RenderHASRawMat = props => {
    ...
    <Table.Row >
     <Table.Cell>{x.hc_file_name}</Table.Cell>
     ...
     <Button.Group basic floated='right' size='small'>
      ...
      <Popup content='Padam Fail' position='top center'  trigger={<Button onClick={()=>deleteChecklist(x.id)} icon='trash alternate' />} />
       ...
       {/* <Button size='mini' compact basic float='right'color="teal" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.hc_file_name} target="_blank">Muat Turun</Button>
     <Button size='mini' compact basic color="red" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.hc_file_name} target="_blank">Padam</Button> */}
     </Table.Cell>
   </Table.Row>
    );
    return <Table>
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Senarai Semakan HAS</Header> 
        ...
        <Form.Input
         ...
         <Form.Input
          ...
          {HASChecklist &&
            <RenderHASRawMat data={HASChecklist}/>
          }
        <Divider>
      {/* <Header as='h4'>
