...
const TabHAStor = () => {
  ...
  const {HAStor} = useContext(UploadHASContext);
  let { path, url } = useRouteMatch();
  const [tor_filename, setfilename] = React.useState(null);
  const [date, setdate] = React.useState(null);
  const [refno, setrefno] = React.useState(null);

  const uploadFile=()=>{
    const formData = new FormData(); 
    formData.append("HAStor",tor_filename); 
    formData.append("date",date); 
    formData.append("refno",refno);
    // Details of the uploaded file   
    postHASTor(formData).then(x=>{
      console.log(x)
  ...
  const RenderHAS = props => {
    ...
    <Table.Row >
     <Table.Cell>{x.tor_filename}</Table.Cell>
     ...
     <Button.Group basic floated='right' size='small'>
      <Popup content='Muat Turun Fail'position='top center' trigger={<Button href={"/files/HASFILE/"+x.cmpnyFK+"/"+x.tor_filename} target="_blank"  icon='download' />} />
      <Popup content='Padam Fail' position='top center'  trigger={<Button onClick={()=>deleteFile(x.id)} icon='trash alternate' />} />
        
      </Button.Group>
       {/* <Button size='mini' compact basic float='right'color="teal" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.tor_filename} target="_blank">Muat Turun</Button>
     <Button size='mini' compact basic color="red" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.tor_filename} target="_blank">Padam</Button> */}
     </Table.Cell>
   </Table.Row>
    );
    return <Table>
