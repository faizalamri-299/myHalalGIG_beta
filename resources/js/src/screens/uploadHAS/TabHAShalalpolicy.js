...
const TabHAShalalpolicy = () => {
  
  const {HAShalalpolicy} = useContext(UploadHASContext);
  let { path, url } = useRouteMatch();
  const [halalpolicy_filename, setfilename] = React.useState(null);
  const [date, setdate] = React.useState(null);
  const [refno, setrefno] = React.useState(null);

  const uploadFile=()=>{
    const formData = new FormData(); 
    formData.append("HAShalalpolicy",halalpolicy_filename);
    formData.append("date",date); 
    formData.append("refno",refno); 
    // Details of the uploaded file   
    postHASHalalpolicy(formData).then(x=>{
      console.log(x)
  ...
  const RenderHAS = props => {
    ...
    <Table.Row >
     <Table.Cell>{x.halalpolicy_filename}</Table.Cell>
     ...
     <Button.Group basic floated='right' size='small'>
      ...
      <Popup content='Padam Fail' position='top center'  trigger={<Button onClick={()=>deleteFile(x.id)} icon='trash alternate' />} />
       ...
       {/* <Button size='mini' compact basic float='right'color="teal" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.halalpolicy_filename} target="_blank">Muat Turun</Button>
     <Button size='mini' compact basic color="red" href={"/files/HASFILE/"+x.hrm_fk_company_id+"/RawMaterial/"+x.halalpolicy_filename} target="_blank">Padam</Button> */}
     </Table.Cell>
   </Table.Row>
    );
    return <Table>
...
export default TabHAShalalpolicy