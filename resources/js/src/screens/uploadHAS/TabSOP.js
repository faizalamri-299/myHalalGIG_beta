...
const TabHASSOP = () => {
  ...
  const {HASSOP} = useContext(UploadHASContext);
  ...
  const uploadFile=()=>{
    ...
    formData.append("HASSOP",filename); 
    formData.append("date",date); 
    formData.append("refno",refno); 
    // Details of the uploaded file   
    postHASSOP(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHASSOP(pk).then(console.log(pk));
  }
