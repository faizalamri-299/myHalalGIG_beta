...
import {UploadHalalContext,
        postHALALPengilanganProduk,
        deleteHALALPengilanganProduk,
        downloadHALALPengilanganProduk} from './UploadHalal'; //tukar ni
...
const TabPengilanganProduk = () => { //tukar ni

  
  const {HALALPengilanganProduk} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALPengilanganProduk",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALPengilanganProduk(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALPengilanganProduk(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALPengilanganProduk(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Rekod Pengilangan Produk</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALPengilanganProduk &&
            <RenderHASRawMat data={HALALPengilanganProduk}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabPengilanganProduk