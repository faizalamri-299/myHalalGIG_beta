...
import {UploadHalalContext,
        postHALALPengesananHalal,
        deleteHALALPengesananHalal,
        downloadHALALPengesananHalal} from './UploadHalal'; //tukar ni
...
const TabPengesananHalal = () => { //tukar ni

  
  const {HALALPengesananHalal} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALPengesananHalal",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALPengesananHalal(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALPengesananHalal(pk).then(console.log(pk));
  }
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Rekod Sistem Pengesanan Halal</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALPengesananHalal &&
            <RenderHASRawMat data={HALALPengesananHalal}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabPengesananHalal