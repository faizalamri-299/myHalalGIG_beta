...
import {UploadHalalContext,
        postHALALKumpulanPembuatan,
        deleteHALALKumpulanPembuatan,
        downloadHALALKumpulanPembuatan} from './UploadHalal'; //tukar ni
...
const TabKumpulanPembuatan = () => { //tukar ni

  
  const {HALALKumpulanPembuatan} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALKumpulanPembuatan",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALKumpulanPembuatan(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALKumpulanPembuatan(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALKumpulanPembuatan(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Rekod Kumpulan Pembuatan</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALKumpulanPembuatan &&
            <RenderHASRawMat data={HALALKumpulanPembuatan}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabKumpulanPembuatan