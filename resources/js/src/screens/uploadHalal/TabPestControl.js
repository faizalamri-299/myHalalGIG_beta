...
import {UploadHalalContext,
        postHALALPestControl,
        deleteHALALPestControl,
        downloadHALALPestControl} from './UploadHalal'; //tukar ni
...
const TabPestControl = () => { //tukar ni

  
  const {HALALPestControl} = useContext(UploadHalalContext); //tukar ni
  let { path, url } = useRouteMatch();
  const [hpc_filename, setFilename] = React.useState(null);
  const [hpc_date, setdate] = React.useState(null);
  const [hpc_refno, setrefno] = React.useState(null);

  const uploadFileChecklist=()=>{
    const formData = new FormData(); 
    formData.append("HALALPestControl",hpc_filename); //tukar ni
    formData.append("hpc_date",hpc_date);                  //tukar ni
    formData.append("hpc_refno",hpc_refno);                //tukar ni
    // Details of the uploaded file   
    postHALALPestControl(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALPestControl(pk).then(console.log(pk));
  }
  ...
  const RenderHASRawMat = props => {
    ...
    <Table.Row >
    <Table.Cell>{x.hpc_filename}</Table.Cell>
    <Table.Cell>{x.hpc_refno}</Table.Cell>
    <Table.Cell>{moment(x.hpc_date).format('DD/MM/YYYY hh:mm:ss')}</Table.Cell>
    <Table.Cell>
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        ...
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALPestControl &&
            <RenderHASRawMat data={HALALPestControl}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
