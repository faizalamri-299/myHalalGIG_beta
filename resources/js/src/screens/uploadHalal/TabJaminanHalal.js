...
import {UploadHalalContext,
        postHALALJaminanHalal,
        deleteHALALJaminanHalal,
        downloadHALALJaminanHalal} from './UploadHalal'; //tukar ni
...
const TabJaminanHalal = () => { //tukar ni

  
  const {HALALJaminanHalal} = useContext(UploadHalalContext); //tukar ni
  let { path, url } = useRouteMatch();
  const [hjh_filename, setFilename] = React.useState(null);
  const [hjh_date, setdate] = React.useState(null);
  const [hjh_refno, setrefno] = React.useState(null);

  const uploadFileChecklist=()=>{
    const formData = new FormData(); 
    formData.append("HALALJaminanHalal",hjh_filename); //tukar ni
    formData.append("hjh_date",hjh_date);                  //tukar ni
    formData.append("hjh_refno",hjh_refno);                //tukar ni
    // Details of the uploaded file   
    postHALALJaminanHalal(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALJaminanHalal(pk).then(console.log(pk));
  }
  ...
  const RenderHASRawMat = props => {
    ...
    <Table.Row >
    <Table.Cell>{x.hjh_filename}</Table.Cell>
    <Table.Cell>{x.hjh_refno}</Table.Cell>
    <Table.Cell>{moment(x.hjh_date).format('DD/MM/YYYY hh:mm:ss')}</Table.Cell>
    <Table.Cell>
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Sistem Jaminan Halal Lengkap</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALJaminanHalal &&
            <RenderHASRawMat data={HALALJaminanHalal}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabJaminanHalal