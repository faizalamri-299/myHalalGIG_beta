...
import {UploadHalalContext,
        postHALALPetaLokasi,
        deleteHALALPetaLokasi,
        downloadHALALPetaLokasi} from './UploadHalal'; //tukar ni
...
const TabPetaLokasi = () => { //tukar ni

  
  const {HALALPetaLokasi} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALPetaLokasi",hpl_filename); //tukar ni
    formData.append("hpl_date",hpl_date);                  //tukar ni
    formData.append("hpl_refno",hpl_refno);                //tukar ni
    // Details of the uploaded file   
    postHALALPetaLokasi(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALPetaLokasi(pk).then(console.log(pk));
  }
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Peta Lokasi Ke Kilang</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALPetaLokasi &&
            <RenderHASRawMat data={HALALPetaLokasi}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
