...
import {UploadHalalContext,
        postHALALDokumenMS,
        deleteHALALDokumenMS,
        downloadHALALDokumenMS} from './UploadHalal'; //tukar ni
...
const TabDokumenMS = () => { //tukar ni

  
  const {HALALDokumenMS} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALDokumenMS",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALDokumenMS(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALDokumenMS(pk).then(console.log(pk));
  }
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Dokumen Berkaitan MS 2400:2019</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALDokumenMS &&
            <RenderHASRawMat data={HALALDokumenMS}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
