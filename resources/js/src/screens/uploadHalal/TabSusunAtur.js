...
import {UploadHalalContext,
        postHALALSusunAtur,
        deleteHALALSusunAtur,
        downloadHALALSusunAtur} from './UploadHalal'; //tukar ni
...
const TabSusunAtur = () => { //tukar ni

  
  const {HALALSusunAtur} = useContext(UploadHalalContext); //tukar ni
  let { path, url } = useRouteMatch();
  const [hsa_filename, setFilename] = React.useState(null);
  const [hsa_date, setdate] = React.useState(null);
  const [hsa_refno, setrefno] = React.useState(null);

  const uploadFileChecklist=()=>{
    const formData = new FormData(); 
    formData.append("HALALSusunAtur",hsa_filename); //tukar ni
    formData.append("hsa_date",hsa_date);                  //tukar ni
    formData.append("hsa_refno",hsa_refno);                //tukar ni
    // Details of the uploaded file   
    postHALALSusunAtur(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALSusunAtur(pk).then(console.log(pk));
  }
  ...
  const RenderHASRawMat = props => {
    ...
    <Table.Row >
    <Table.Cell>{x.hsa_filename}</Table.Cell>
    <Table.Cell>{x.hsa_refno}</Table.Cell>
    <Table.Cell>{moment(x.hsa_date).format('DD/MM/YYYY hh:mm:ss')}</Table.Cell>
    <Table.Cell>
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Pelan Susun Atur Kilang/Premis</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALSusunAtur &&
            <RenderHASRawMat data={HALALSusunAtur}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
