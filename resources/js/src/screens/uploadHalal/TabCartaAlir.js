...
import {UploadHalalContext,
        postHALALCartaAlir,
        deleteHALALCartaAlir,
        downloadHALALCartaAlir} from './UploadHalal'; //tukar ni
...
const TabCartaAlir = () => { //tukar ni

  
  const {HALALCartaAlir} = useContext(UploadHalalContext); //tukar ni
  let { path, url } = useRouteMatch();
  const [hca_filename, setFilename] = React.useState(null);
  const [hca_date, setdate] = React.useState(null);
  const [hca_refno, setrefno] = React.useState(null);

  const uploadFileChecklist=()=>{
    const formData = new FormData(); 
    formData.append("HALALCartaAlir",hca_filename); //tukar ni
    formData.append("hca_date",hca_date);                  //tukar ni
    formData.append("hca_refno",hca_refno);                //tukar ni
    // Details of the uploaded file   
    postHALALCartaAlir(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALCartaAlir(pk).then(console.log(pk));
  }
  ...
  const RenderHASRawMat = props => {
    ...
    <Table.Row >
    <Table.Cell>{x.hca_filename}</Table.Cell>
    <Table.Cell>{x.hca_refno}</Table.Cell>
    <Table.Cell>{moment(x.hca_date).format('DD/MM/YYYY hh:mm:ss')}</Table.Cell>
    <Table.Cell>
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Carta Alir Proses Pengilangan Produk</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALCartaAlir &&
            <RenderHASRawMat data={HALALCartaAlir}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
