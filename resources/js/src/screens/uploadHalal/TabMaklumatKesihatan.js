...
import {UploadHalalContext,
        postHALALMaklumatKesihatan,
        deleteHALALMaklumatKesihatan,
        downloadHALALMaklumatKesihatan} from './UploadHalal'; //tukar ni
...
const TabMaklumatKesihatan = () => { //tukar ni

  
  const {HALALMaklumatKesihatan} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALMaklumatKesihatan",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALMaklumatKesihatan(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALMaklumatKesihatan(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALMaklumatKesihatan(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Rekod Maklumat Kesihatan Pekerja</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALMaklumatKesihatan &&
            <RenderHASRawMat data={HALALMaklumatKesihatan}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabMaklumatKesihatan