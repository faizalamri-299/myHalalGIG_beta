...
import {UploadHalalContext,
        postHALALSuratPerakuanJPV,
        deleteHALALSuratPerakuanJPV,
        downloadHALALSuratPerakuanJPV} from './UploadHalal'; //tukar ni
...
const TabSuratPerakuanJPV = () => { //tukar ni

  
  const {HALALSuratPerakuanJPV} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALSuratPerakuanJPV",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALSuratPerakuanJPV(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALSuratPerakuanJPV(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALSuratPerakuanJPV(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Surat Perakuan JPV</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALSuratPerakuanJPV &&
            <RenderHASRawMat data={HALALSuratPerakuanJPV}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabSuratPerakuanJPV