...
import {UploadHalalContext,
        postHALALRekodSembelihan,
        deleteHALALRekodSembelihan,
        downloadHALALRekodSembelihan} from './UploadHalal'; //tukar ni
...
const TabRekodSembelihan = () => { //tukar ni

  
  const {HALALRekodSembelihan} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALRekodSembelihan",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALRekodSembelihan(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALRekodSembelihan(pk).then(console.log(pk));
  }
      ...
      <div className="in innerContainer">
        ...
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALRekodSembelihan &&
            <RenderHASRawMat data={HALALRekodSembelihan}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
