...
import {UploadHalalContext,
        postHALALLesenPergudangan,
        deleteHALALLesenPergudangan,
        downloadHALALLesenPergudangan} from './UploadHalal'; //tukar ni
...
const TabLesenPergudangan = () => { //tukar ni

  
  const {HALALLesenPergudangan} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALLesenPergudangan",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALLesenPergudangan(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALLesenPergudangan(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALLesenPergudangan(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Lesen Pergudangan</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALLesenPergudangan &&
            <RenderHASRawMat data={HALALLesenPergudangan}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabLesenPergudangan