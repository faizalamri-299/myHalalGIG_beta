...
import {UploadHalalContext,
        postHALALLesenPengilang,
        deleteHALALLesenPengilang,
        downloadHALALLesenPengilang} from './UploadHalal'; //tukar ni
...
const TabLesenPengilang = () => { //tukar ni

  
  const {HALALLesenPengilang} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALLesenPengilang",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALLesenPengilang(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALLesenPengilang(pk).then(console.log(pk));
  }
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Lesen Pengilang/Pengimport (BPFK)</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALLesenPengilang &&
            <RenderHASRawMat data={HALALLesenPengilang}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabLesenPengilang