...
import {UploadHalalContext,
        postHALALSuratKKM,
        deleteHALALSuratKKM,
        downloadHALALSuratKKM} from './UploadHalal'; //tukar ni
...
const TabSuratKKM = () => { //tukar ni

  
  const {HALALSuratKKM} = useContext(UploadHalalContext); //tukar ni
  let { path, url } = useRouteMatch();
  const [hsk_filename, setFilename] = React.useState(null);
  const [hsk_date, setdate] = React.useState(null);
  const [hsk_refno, setrefno] = React.useState(null);

  const uploadFileChecklist=()=>{
    const formData = new FormData(); 
    formData.append("HALALSuratKKM",hsk_filename); //tukar ni
    formData.append("hsk_date",hsk_date);                  //tukar ni
    formData.append("hsk_refno",hsk_refno);                //tukar ni
    // Details of the uploaded file   
    postHALALSuratKKM(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALSuratKKM(pk).then(console.log(pk));
  }
  ...
  const RenderHASRawMat = props => {
    ...
    <Table.Row >
    <Table.Cell>{x.hsk_filename}</Table.Cell>
    <Table.Cell>{x.hsk_refno}</Table.Cell>
    <Table.Cell>{moment(x.hsk_date).format('DD/MM/YYYY hh:mm:ss')}</Table.Cell>
    <Table.Cell>
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Surat Kebenaran Pihak Berkuasa Kawalan Dadah KKM</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALSuratKKM &&
            <RenderHASRawMat data={HALALSuratKKM}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
