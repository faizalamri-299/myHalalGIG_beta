...
import {UploadHalalContext,
        postHALALPerakuanProduk,
        deleteHALALPerakuanProduk,
        downloadHALALPerakuanProduk} from './UploadHalal'; //tukar ni
...
const TabPerakuanProduk = () => { //tukar ni

  
  const {HALALPerakuanProduk} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALPerakuanProduk",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALPerakuanProduk(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALPerakuanProduk(pk).then(console.log(pk));
  }
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Sijil Perakuan Pendaftaran Produk (BPFK), KKM</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALPerakuanProduk &&
            <RenderHASRawMat data={HALALPerakuanProduk}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabPerakuanProduk