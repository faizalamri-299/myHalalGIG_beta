...
import {UploadHalalContext,
        postHALALAliranPergerakan,
        deleteHALALAliranPergerakan,
        downloadHALALAliranPergerakan} from './UploadHalal'; //tukar ni
...
const TabAliranPergerakan = () => { //tukar ni

  
  const {HALALAliranPergerakan} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALAliranPergerakan",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALAliranPergerakan(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALAliranPergerakan(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALAliranPergerakan(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Aliran Pergerakan Operasi</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALAliranPergerakan &&
            <RenderHASRawMat data={HALALAliranPergerakan}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabAliranPergerakan