...
import {UploadHalalContext,
        postHALALRekodSertu,
        deleteHALALRekodSertu,
        downloadHALALRekodSertu} from './UploadHalal'; //tukar ni
...
const TabRekodSertu = () => { //tukar ni

  
  const {HALALRekodSertu} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALRekodSertu",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALRekodSertu(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALRekodSertu(pk).then(console.log(pk));
  }
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        ...
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALRekodSertu &&
            <RenderHASRawMat data={HALALRekodSertu}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
