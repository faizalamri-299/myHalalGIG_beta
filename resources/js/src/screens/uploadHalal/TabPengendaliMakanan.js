...
import {UploadHalalContext,
        postHALALPengendaliMakanan,
        deleteHALALPengendaliMakanan,
        downloadHALALPengendaliMakanan} from './UploadHalal'; //tukar ni
...
const TabPengendaliMakanan = () => { //tukar ni

  
  const {HALALPengendaliMakanan} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALPengendaliMakanan",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALPengendaliMakanan(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALPengendaliMakanan(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALPengendaliMakanan(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Sijil Kursus Pengendali Makanan</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALPengendaliMakanan &&
            <RenderHASRawMat data={HALALPengendaliMakanan}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabPengendaliMakanan