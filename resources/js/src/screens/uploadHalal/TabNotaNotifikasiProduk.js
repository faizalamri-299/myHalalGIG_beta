...
import {UploadHalalContext,
        postHALALNotaNotifikasiProduk,
        deleteHALALNotaNotifikasiProduk,
        downloadHALALNotaNotifikasiProduk} from './UploadHalal'; //tukar ni
...
const TabNotaNotifikasiProduk = () => { //tukar ni

  
  const {HALALNotaNotifikasiProduk} = useContext(UploadHalalContext); //tukar ni
  ...
  const uploadFileChecklist=()=>{
    ...
    formData.append("HALALNotaNotifikasiProduk",filename); //tukar ni
    formData.append("date",date);                  //tukar ni
    formData.append("refno",refno);                //tukar ni
    // Details of the uploaded file   
    postHALALNotaNotifikasiProduk(formData).then(x=>{
      console.log(x)
    }).catch(e=>console.log(e))
  }

  const downloadfile=(pk)=>{
    downloadHALALNotaNotifikasiProduk(pk).then(console.log(pk));
  }
    ...
    .then((willDelete) => {
      if (willDelete) {
        swal("Item telah dipadam!", {
          ...
          console.log(result);
          if(result) {
            deleteHALALNotaNotifikasiProduk(pk)
  ...
  return (
    ...
    <Transition transitionOnMount={true} animation="fade" duration={1000}>
      <div className="in innerContainer">
        <Header as='h3'>Nota Notifikasi Produk Dikeluarkan BPFK</Header> 
        <Form.Group style={{display:'flex'}}>
          ...
          {HALALNotaNotifikasiProduk &&
            <RenderHASRawMat data={HALALNotaNotifikasiProduk}/> //tukar ni
          }
        <Divider>
    </Divider>
      </div>
...
export default TabNotaNotifikasiProduk